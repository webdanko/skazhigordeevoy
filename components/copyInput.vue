  ...
  <div class="w-full">
    <div class="relative">
      ...
      <div
        :id="`tooltip-${id}`"
        role="tooltip"
        class="copy-clipboard-tooltip tooltip invisible opacity-0"
      >
        <span :id="`default-tooltip-message-${id}`">Скопировать</span>
        <span
          :id="`success-tooltip-message-${id}`"
          class="hidden"
        >Скопировано!</span>
        <div
          class="tooltip-arrow"
...
<style scoped lang="postcss">
  .copy-clipboard-input {
    @apply col-span-6 bg-gray-50 border border-gray-300 text-gray-500 text-sm overflow-ellipsis rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 pr-10;
  }

  .copy-clipboard-button {
    @apply absolute end-2 top-1/2 -translate-y-1/2 text-gray-500 hover:bg-gray-100 rounded-lg p-2 inline-flex items-center justify-center;
  }

  .copy-clipboard-tooltip {
    @apply absolute z-10 inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm;
  }
</style>